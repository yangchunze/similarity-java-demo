...
public class CosineSimilarity {
    ...
    protected static void taggingWeightByFrequency(List<Word> words1, List<Word> words2) {
        ...
        Map<String, AtomicFloat> frequency1 = getFrequency(words1);
        Map<String, AtomicFloat> frequency2 = getFrequency(words2);
        //如果是DEBUG模式输出词频统计信息
     ...
     * @return 词频统计图
     */
    private static Map getFrequency(List<Word> words) {
        Map<String, AtomicFloat> freq = new HashMap<>();
        //这步很帅哦
        words.forEach(i -> freq.computeIfAbsent(i.getName(), k -> new AtomicFloat()).incrementAndGet());
        return freq;
    }
     ...
     * 构造权重快速搜索容器
     */
    protected static Map getFastSearchMap(List<Word> words) {
        if (CollectionUtils.isEmpty(words)) {
            return Collections.emptyMap();
        }
        Map weightMap = new ConcurrentHashMap<>(words.size());
